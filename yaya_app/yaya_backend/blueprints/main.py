...
# MANUALS
# =========================================================
@main.route('/manuals', methods=['GET'])
def manuals():
    ...
    return jsonify({
        "manuals": [{
            ...
            "download_url": f"/static/uploads/manuals/{m.filename}" if m.filename else None
...
@role_required('superadmin')
def view_logs():
    all_logs = Log.query.order_by(Log.id.desc()).limit(100).all()
...
@main.route('/api/v1/banking-details', methods=['GET'])
def api_get_banking_details():
    tithe_bank = BankDetail.query.filter_by(category='Tithe').first()
    project_bank = BankDetail.query.filter_by(category='Project').first()

    return jsonify({
        "tithe": {
            "bank_name": tithe_bank.bank_name if tithe_bank else "N/A",
            "account_number": tithe_bank.account_number if tithe_bank else "N/A",
            "account_name": tithe_bank.account_name if tithe_bank else "RCCG YAYA"
        },
        "project": {
            "bank_name": project_bank.bank_name if project_bank else "N/A",
            "account_number": project_bank.account_number if project_bank else "N/A",
            "account_name": project_bank.account_name if project_bank else "RCCG YAYA"
        }
    }), 200
...
@main.route("/live-stream")
def live_stream_page():
    streams = LiveStream.query.all()
    return render_template("live_stream.html", streams=streams)


@main.route('/api/v1/feed', methods=['GET'])
def api_get_feed():
    # Flutter will send an optional ?branch_id=XX query parameter
    selected_branch_id = request.args.get('branch_id', type=int)

    # Query data structures identical to your desktop logic
    all_branches = Branch.query.order_by(Branch.name.asc()).all()
    main_stream = Stream.query.order_by(Stream.created_at.desc()).first()

    branch_stream_url = None
    if selected_branch_id:
        branch = Branch.query.get(selected_branch_id)
        if branch:
            branch_stream_url = branch.youtube_link

    # Filter posts based on the branch choice
    if selected_branch_id:
        posts = Post.query.filter((Post.is_global == True) | (Post.branch_id == selected_branch_id)).order_by(Post.id.desc()).all()
    else:
        posts = Post.query.filter_by(is_global=True).order_by(Post.id.desc()).all()
    ...
    return jsonify({
        "main_stream_url": main_stream.url if main_stream else None,
        "branch_stream_url": branch_stream_url,
        "branches": [{"id": b.id, "name": b.name, "location": b.location} for b in all_branches],
        "announcements": [{
            "id": a.id,
            "title": a.title,
            "content": a.content,
            "image_url": f"/static/uploads/{a.image}" if a.image else None,
            "video_url": a.video or (f"/static/uploads/{a.video_file}" if a.video_file else None)
